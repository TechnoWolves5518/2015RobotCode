...
public class DriveTrain extends RobotFunction {
	...
	@Override
	public void start() {
		while (Robot.jOi.getJoystick().getRawButton(RobotMap.BTN_TRIGGER)){
			m_robot.mecanumDrive_Cartesian(Robot.jOi.getJoystick().getRawAxis(RobotMap.X_AXIS)*.5,
					Robot.jOi.getJoystick().getRawAxis(RobotMap.Y_AXIS)*.5, 
					Robot.jOi.getJoystick().getRawAxis(RobotMap.Z_AXIS)*.5, 0);
		}
